"""
DETR Fine-tuning on AU-AIR Dataset
...
class AUAIRDataset(Dataset):
    def __init__(self, annotations_file, img_dir, processor, split="train"):
        """
        Args:
        ...
        self.categories = data.get("categories", CLASS_NAMES)
        ...
        if split == "train":
            self.annotations = self.annotations[:train_size]
        elif split == "val":
            self.annotations = self.annotations[train_size:train_size+val_size]
        else:  # test
            self.annotations = self.annotations[train_size+val_size:]
    ...
    def __getitem__(self, idx):
        ...
        for box in annotation.get("bbox", []):
            try:
                x_min = box.get("left", 0)
                y_min = box.get("top", 0)
                width = box.get("width", 0)
                height = box.get("height", 0)
                class_idx = box.get("class", 0)
                ...
                print(f"Error processing box: {box}, Error: {e}")
                continue
        ...
        try:
                ...
                "annotations": [
                    {
                        "bbox": [box[0].item(), box[1].item(),
                                (box[2] - box[0]).item(), (box[3] - box[1]).item()],  # COCO format: [x, y, width, height]
            ...
            encoding["boxes"] = boxes
            encoding["labels"] = class_labels
            encoding["class_labels"] = class_labels  # Add class_labels for the DETR matcher
            encoding["image_id"] = torch.tensor(idx)
            encoding["area"] = (boxes[:, 2] - boxes[:, 0]) * (boxes[:, 3] - boxes[:, 1])
            encoding["iscrowd"] = torch.zeros_like(class_labels)

            # Add original image size for visualization
            encoding["orig_size"] = torch.tensor([image.height, image.width])
...
def create_data_loaders(processor):
    ...
    train_dataset = AUAIRDataset(ANNOTATIONS_FN, IMAGES_DIR, processor, split="train")
    val_dataset = AUAIRDataset(ANNOTATIONS_FN, IMAGES_DIR, processor, split="val")
    test_dataset = AUAIRDataset(ANNOTATIONS_FN, IMAGES_DIR, processor, split="test")
...
def collate_fn(batch):
    ...
    for item in batch:
        ...
        target = {
            "boxes": item["boxes"],
            "labels": item["labels"],
            "class_labels": item["class_labels"] if "class_labels" in item else item["labels"],  # Ensure class_labels is present
            "image_id": item["image_id"] if "image_id" in item else torch.tensor(0),
            "area": item["area"] if "area" in item else (item["boxes"][:, 2] - item["boxes"][:, 0]) * (item["boxes"][:, 3] - item["boxes"][:, 1]),
            "iscrowd": item["iscrowd"] if "iscrowd" in item else torch.zeros_like(item["labels"])
        }
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(len(orig_images)):
                ...
                gt_labels = batch["labels"][i]["labels"].cpu()
